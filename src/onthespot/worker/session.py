...
class LoadSessions(QObject):
    ...
    def run(self):
        ...
        for account in accounts:
            ...
            self.progress.emit(self.tr('Attempting to create session...'), True)
            time.sleep(0.2)
            ...
            if login is not None:
                if login[0]:
                    ...
                    self.progress.emit(self.tr('Session created!'), True)
                    time.sleep(0.2)
                    session_pool[account[3]] = login[1]
                    self.__users.append([account[0], 'Premium' if login[3] else 'Free', 'OK', account[3]])
                    logged_in = True
            if not logged_in:
                self.progress.emit(self.tr('Failed to create session.'), True)
